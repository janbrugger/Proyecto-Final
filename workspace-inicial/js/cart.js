...
const datos = JSON.parse(localStorage.getItem("productosSeleccionados"))
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  for (const dato of datos) {
    container.innerHTML += `
    <tr>
      <td><img onclick="setProductID(${dato.id})" src="${dato.images[1]}" class="img-fluid mt-2 cursor-active" style="max-height: 80px;"></img></td>
      <td>${dato.name}</td>
      <td>${dato.currency} <span>${dato.cost}</span></td>
      <td><input class="col-lg-2"  value=${"article.count"}></td>
      <td><strong>${"article.currency"} <span>${"article.unitCost * article.count"}</span><strong></td>
    </tr>
    `
